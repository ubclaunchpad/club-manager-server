...
describe("Applicant's ScreeningGrade + InterviewGrade Get", () => {
  ...
  test("GET /applicant", async () => {
    const applicant = await Applicant.create({
      ...
      role: "Developer", major: "Combined CS + Stat", yearStanding: 1,
      level: "Beginner", status: "Pending"
    });

    await supertest(server).get("/applicant")
      .expect(201)
      .then((response) => {
  ...
  test("GET /grade/screening/:applicantId", async () => {
    ...
    const screeningGrade = await ScreeningGrade.create({ applicant: applicant._id, c1: 1, c2: 1, c3: 1, c4: 1, c5: 1, c6: 1, total: 6 });
    const applicantId = screeningGrade.applicant.toString()

    await supertest(server).get("/grade/screening/" + applicantId)
      .expect(201)
      .then((response) => {
        // Check data
  ...
  test("GET /grade/interview/:applicantId", async () => {
    ...
    await supertest(server).get("/grade/interview/" + applicantId)
      .expect(201)
      .then((response) => {
        // Check data
